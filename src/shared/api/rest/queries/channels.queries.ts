...
export const channelsQueries = {
  create: (name: string, icon: File) => {
    return APIInterface.post<Channel>(
  ...
  get: (channelIds: ID | ID[], withMembers = true) => {
    return APIInterface.get<Channel[]>(API_ROUTES.CHANNELS.GET, {
  ...
  updateChannel: (channelId: ID, data: Partial<Omit<Channel, 'banner' | 'icon'> & { icon?: File; banner?: File }>) => {
    ...
    return APIInterface.patch<Channel>(`${API_ROUTES.CHANNELS.UPDATE_CHANNEL}/${channelId}`, rest, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
  },
  leaveChannel: (channelId: string) => {
    return APIInterface.get(`${API_ROUTES.CHANNELS.LEAVE}`, { params: { channelId } });
  },
};
